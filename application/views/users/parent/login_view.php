...
<div class="w3-margin"><b class="w3-xlarge w3-text-teal w3-margin w3-serif">PARENTS LOGIN</b></div> <br>
<span class="w3-block w3-text-red"><?=validation_errors() ?></span>
<?= isset($_SESSION['action_status_report'])? $_SESSION['action_status_report']:"" ?>
<?= form_open('parents/login/'.$this->uri->segment(3))?>

<span class="w3-label">Phone Number</span>
<input type="text" name="phone" placeholder="Phone(070xxxxxxxxx)" class="w3-input w3-margin" value="<?=empty($this->uri->segment(3))? "": strtoupper($this->uri->segment(3)) ?>" />
<span class="w3-label">Parent Password</span>
<br>
...
<br>
<a href="<?=site_url('page/parent_forgot_password') ?>"><i class="w3-text-teal">Forget Password?</i></a>
</div>	
</center>
